...
| Area | What now exists |
| --- | --- |
| Settlement economics | `SettlementEscrow.sol` holds payment and a provider bond and releases only against a physical (A3) verdict. Both sides commit to both stake sizes before a deal activates, deals are namespaced by both parties so a stranger cannot squat or brick a slot, a missing verdict holds funds for arbitration instead of auto-refunding, a borderline failure disputes rather than slashes, a late but correct verdict can still settle while arbitration is open, and an expired arbitration unwinds neutrally. Value conservation is asserted exactly, and every terminal path plus the squatting, one-wei-counterparty, and bond-floor attacks are covered by tests. |
| Governance | `MultiSigTimelock.sol` puts privileged actions behind M-of-N approval plus a delay. Cancellation is restricted to guardians, because letting any signer cancel would turn an M-of-N multisig into a 1-of-N veto, and a cancelled action can be proposed again from a clean slate. Signer rotation, threshold, and delay changes are callable only through the timelock itself, so a lost key can be replaced under the same rules. [GOVERNANCE.md](GOVERNANCE.md) defines roles, separation of duties, validator membership, disputes, and upgrades. |
